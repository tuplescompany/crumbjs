	...
	if (createEndpoints.post) {
		...
		resource.post(
			...
			async (c) => {
				...
				c.body.createdAt = new Date();
				c.body.updatedAt = null;
				c.body.deletedAt = null;
				const created = await repository.create(c.body as any, false); // Repo validation/parse disabled
	...
	if (createEndpoints.put) {
		...
		resource.put(
			...
			async (c) => {
				...
				const updated = await repository.updateOne(filters, c.body as any, false); // Repo validation/parse disabled
