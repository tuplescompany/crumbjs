...
const DEFAULT_EXCLUDE_DIRS = new Set([
  'node_modules','dist','build','.next','.nuxt','.git','coverage','out','.output','.turbo','.vercel', 'logo'
]);
...
async function main() {
  ...
  const rows = [...byLang.entries()]
    ...
    .map(([lang, {files, lines}]) => ({ lang, files, lines }));
