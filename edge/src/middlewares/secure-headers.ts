...
export type ContentSecurityPolicyOptionHandler = (ctx: MiddlewareContext, directive: string) => string;
 ...
 * Ported from Hono @see {@link https://hono.dev/docs/middleware/builtin/secure-headers}
 *
 ...
 * ```
 */
export const secureHeaders = (customOptions?: SecureHeadersOptions): Middleware => {
	...
	if (options.permissionsPolicy && Object.keys(options.permissionsPolicy).length > 0) {
		headersToSet.push(['Permissions-Policy', getPermissionsPolicyDirectives(options.permissionsPolicy)]);
	}
	...
	return async function secureHeaders(ctx) {
...
function getCSPDirectives(contentSecurityPolicy: ContentSecurityPolicyOptions): [SecureHeadersCallback | undefined, string | string[]] {
	const callbacks: ((ctx: MiddlewareContext, values: string[]) => void)[] = [];
	...
	for (const [directive, value] of Object.entries(contentSecurityPolicy)) {
		...
		resultValues.push(
			directive.replace(/[A-Z]+(?![a-z])|[A-Z]/g, (match, offset) => (offset ? '-' + match.toLowerCase() : match.toLowerCase())),
...
function getReportingEndpoints(reportingEndpoints: SecureHeadersOptions['reportingEndpoints'] = []): string {
...
function setHeaders(ctx: MiddlewareContext, headersToSet: [string, string][]) {
