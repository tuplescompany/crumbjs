...
export function addRoute<T>(ctx: RouterContext<T>, method: string = '', path: string, data?: T): void {
	...
	for (let i = 0; i < segments.length; i++) {
		...
		if (segment.startsWith('**')) {
			...
			node = node.wildcard;
			paramsMap.push([-i, segment.split(':')[1] || '_', segment.length === 2 /* no id */]);
			break;
		}
		...
		if (segment === '*' || segment.includes(':')) {
			...
			paramsMap.push([i, isOptional ? `_${_unnamedParamIndex++}` : (_getParamMatcher(segment) as string), isOptional]);
			continue;
		}
